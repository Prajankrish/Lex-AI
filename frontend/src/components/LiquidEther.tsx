...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    console.log('LiquidEther useEffect mounted, container:', mountRef.current);
    console.log('Container size:', mountRef.current.clientWidth, 'x', mountRef.current.clientHeight);

    // Wait a frame for layout to settle, then check width again
    requestAnimationFrame(() => {
      if (!mountRef.current) return;
      const actualWidth = mountRef.current.clientWidth;
      const actualHeight = mountRef.current.clientHeight;
      console.log('After RAF - Container actual size:', actualWidth, 'x', actualHeight);
      
      if (actualWidth === 0) {
        console.warn('Container width is still 0! Using parent or fallback dimensions.');
        if (mountRef.current.parentElement) {
          const parentWidth = mountRef.current.parentElement.clientWidth;
    ...
    class CommonClass {
      ...
      init(container: any) {
        ...
        this.resize();
        console.log('Common.resize() done, size:', this.width, this.height);
        this.renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
        console.log('THREE.WebGLRenderer created:', this.renderer);
        this.renderer.autoClear = false;
        this.renderer.setClearColor(new THREE.Color(0x000000), 0);
        this.renderer.setPixelRatio(this.pixelRatio);
        this.renderer.setSize(this.width, this.height);
        console.log('Renderer size set to:', this.width, this.height);
        this.renderer.domElement.style.width = '100%';
        this.renderer.domElement.style.height = '100%';
        this.renderer.domElement.style.display = 'block';
        this.clock = new THREE.Clock();
        this.clock.start();
        console.log('Common.init() complete');
      }
      resize() {
        ...
        // If width is still very small (less than 10px), try parent
        if (width < 10 && this.container.parentElement) {
          const parentRect = this.container.parentElement.getBoundingClientRect();
          width = Math.max(1, Math.floor(parentRect.width));
        }
        ...
        console.log('Common.resize() - final size:', this.width, 'x', this.height);
      }
      update() {
...
`;
    ...
    if(isBFECC == false){
        ...
        vec2 vel_2 = texture2D(velocity, vec2(spot_new3)).xy;
...
`;
    ...
    class Output {
      ...
      init() {
        console.log('Output.init() starting...');
        ...
        console.log('Output.init() complete, output mesh:', this.output);
      }
      addScene(mesh: any) {
        this.scene.add(mesh);
      }
      ...
      render() {
        if (!Common.renderer) {
          console.error('Common.renderer not initialized!');
          return;
        }
        Common.renderer.setRenderTarget(null);
        Common.renderer.render(this.scene, this.camera);
    ...
    console.log('Creating WebGLManager with container:', container);
    ...
    console.log('WebGLManager created:', webgl);
    ...
    // Force a resize after a frame to ensure proper dimensions
    requestAnimationFrame(() => {
      if (webglRef.current && webglRef.current.resize) {
        console.log('Force resize after RAF');
  ...
  return (
    <div
      ...
      style={{
        ...
        position: 'relative',
        overflow: 'hidden',
        background: 'linear-gradient(135deg, #5227FF20 0%, #FF9FFC20 50%, #B19EEF20 100%)',
      }}
    >
      {/* WebGL canvas will be appended here by the component */}
    </div>
  );
}
